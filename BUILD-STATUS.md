...
- **Purpose**: fail the build (exit 1) before `astro build` if authored content breaks schema; runs the same engine plus two static atom checks. Confirmed run (2026-06-08): **9 files valid, 9 atoms in sync; 8 audited schemas as the top-level gate, 18 atom schemas loaded for nested-slot child lookup (atoms only, collision-guarded).**
- **Enforces**: scope gate (top-level validation only for the 9 AUDITED_COMPONENTS); canonical 5-file existence; coarse FORBIDDEN_CSS/FORBIDDEN_ASTRO regex scan; `checkAstroVsSchema` (props in `.astro` missing from schema — reverse drift); data validation of `src/data/test/<name>.json`; **the (B) nested-slot lock on `_lockProps` media slots — Badge/Button/Heading/List media + FormField options[*].media children are deep-validated against the child atom's own schema** (2026-06-08 rollout; dry-run + live run both 0 errors — existing media data was already clean).
- **Scope rule (PINNED 2026-06-08) — what "audited-only" means, exactly:** the top-level tree walk validates **AUDITED atoms only** (`validatePage` is handed the audited-only map). Unaudited nodes that merely scaffold examples (Grid, Section, Page) are **skipped** — they neither pass nor fail an audited atom's test page (the walk still recurses *through* them to reach audited atoms deeper). An audited atom's **`_lockProps` media slot, by contrast, DOES deep-validate its child** (via `__schemaMap` = all atoms) — that's a deliberate composition the atom owns, not arbitrary scaffolding. So: audited atoms gated at the top level + their opted-in media children; everything else in the tree is ignored. The two pull in opposite directions on purpose and are cleanly separated by mechanism (audited-map gate vs `_lockProps`+all-atoms map). Proven + regression-guarded by `npm run test:locks` (scope cases).
- **Gaps (blind spots)**:
...
### `scripts/test-nested-locks.ts` — lock regression test (`npm run test:locks`)
- **Purpose**: adversarial regression test for the (B) nested-slot locks + the scope rule. Feeds deliberate good + bad input through the real `validateComponent` / `validatePage` and exits 1 on any miss. **Chained into `npm run build`** (after `validate:data`), so a broken lock fails the build.
- **Covers (42 cases, 2026-06-08)**: Card slots; FormField `options[*].media` + Button `dropdownItems[*]`; Badge/Button/Heading/List `media` (good incl. child appearance props; wrong-but-real atom; bad child enum; unknown child prop; missing required); Heading Image/Shape + List Shape child paths; and the scope rule (unaudited Grid/Section scaffolding skipped at the top level while the audited atom's media-lock still fires inside it).
- **Not covered**: render output / CSS (these are schema+validator proofs only); atoms with no media slot (nothing to lock).
