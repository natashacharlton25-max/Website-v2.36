...
// .wrangler/tmp/bundle-XQtZm5/strip-cf-connecting-ip-header.js
...
async function handleAssetPatch(request, env, slug) {
  ...
  if (body.alt_symbol_id !== void 0) {
    stmts.push(env.DB.prepare("UPDATE assets SET alt_symbol_id = ?, updated_at = ? WHERE id = ?").bind(body.alt_symbol_id, now, asset.id));
  }
  if (body.alt_descriptive !== void 0) {
    stmts.push(env.DB.prepare("UPDATE assets SET alt_descriptive = ?, updated_at = ? WHERE id = ?").bind(body.alt_descriptive, now, asset.id));
  }
...
__name(formatLicense, "formatLicense");
...
async function handleLicenseGet(_request, env, key) {
  const row = await env.DB.prepare("SELECT * FROM licenses WHERE key = ?").bind(key).first();
  if (!row)
    return jsonError(404, `License not found: ${key}`);
  return json(formatLicense(row));
}
__name(handleLicenseGet, "handleLicenseGet");
...
// src/routes/alt-symbols.ts
async function handleAltSymbolList(request, env) {
  const url = new URL(request.url);
...
__name(handleAltSymbolList, "handleAltSymbolList");
async function handleAltSymbolGet(request, env, id) {
  const row = await env.DB.prepare("SELECT * FROM alt_symbols WHERE id = ?").bind(id).first();
  if (!row)
    return notFound(`Alt symbol not found: ${id}`);
  return json(row);
}
__name(handleAltSymbolGet, "handleAltSymbolGet");
async function handleAltSymbolCreate(request, env) {
  const authErr = requireAuth(request, env);
  if (authErr)
    return authErr;
  const body = await request.json();
...
__name(handleAltSymbolCreate, "handleAltSymbolCreate");
async function handleAltSymbolUpdate(request, env, id) {
  const authErr = requireAuth(request, env);
  if (authErr)
    return authErr;
  const existing = await env.DB.prepare("SELECT id FROM alt_symbols WHERE id = ?").bind(id).first();
  if (!existing)
    return notFound(`Alt symbol not found: ${id}`);
  const body = await request.json();
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const stmts = [];
  if (body.word !== void 0) {
    stmts.push(env.DB.prepare("UPDATE alt_symbols SET word = ?, updated_at = ? WHERE id = ?").bind(body.word, now, id));
  }
  if (body.icon_id !== void 0) {
    stmts.push(env.DB.prepare("UPDATE alt_symbols SET icon_id = ?, updated_at = ? WHERE id = ?").bind(body.icon_id, now, id));
  }
  if (body.aac_id !== void 0) {
    stmts.push(env.DB.prepare("UPDATE alt_symbols SET aac_id = ?, updated_at = ? WHERE id = ?").bind(body.aac_id, now, id));
  }
  if (body.aac_url !== void 0) {
    stmts.push(env.DB.prepare("UPDATE alt_symbols SET aac_url = ?, updated_at = ? WHERE id = ?").bind(body.aac_url, now, id));
  }
  if (stmts.length > 0) {
    await env.DB.batch(stmts);
  }
...
__name(handleAltSymbolUpdate, "handleAltSymbolUpdate");
async function handleAltSymbolDelete(request, env, id) {
  const authErr = requireAuth(request, env);
  if (authErr)
    return authErr;
  const existing = await env.DB.prepare("SELECT id FROM alt_symbols WHERE id = ?").bind(id).first();
...
__name(handleAltSymbolDelete, "handleAltSymbolDelete");
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (altSymbolMatch) {
        ...
        if (method === "GET")
          return handleAltSymbolGet(request, env, id);
        if (method === "PUT")
          return handleAltSymbolUpdate(request, env, id);
        if (method === "DELETE")
          return handleAltSymbolDelete(request, env, id);
        return jsonError(405, "Method not allowed");
      }
      if (path === "/v1/alt-symbols") {
        if (method === "GET")
          return handleAltSymbolList(request, env);
        if (method === "POST")
          return handleAltSymbolCreate(request, env);
        return jsonError(405, "Method not allowed");
      }
      ...
      if (licenseMatch) {
        if (method === "GET")
          return handleLicenseGet(request, env, licenseMatch[1]);
        return jsonError(405, "Method not allowed");
      }
      if (path === "/v1/licenses") {
        if (method === "GET")
...
// .wrangler/tmp/bundle-XQtZm5/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-XQtZm5/middleware-loader.entry.ts
