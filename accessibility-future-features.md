...
| **Dev console welcome message** | Random joke + credits on every page load in dev tools console. 18+ jokes that rotate. Credits section acknowledges open source tools (Astro, chroma-js, GSAP, Phosphor, OpenAAC, BCI, Piccalilli, Cloudflare). CIC philosophy visible to developers inspecting the code. One line in BaseLayout. | `console.log('%c👋 Mind the Box', ...)` + random joke + credits. |
...
| **Caret voice input indicator** | When voice input is active, caret shows a pulsing dot (recording indicator). Caret colour changes to `--color-Success` when speech recognised, `--color-Error` when failed, `--color-Warning` when processing. Dot disappears when voice stops. | Integrates with Voice Input feature. `--caret-dot-color` token. Pulse gated by motion settings. |
| **Persistent bookmark / "where was I"** | Arrow tab saves last position to localStorage (page URL, scroll position, field index/ID). On reload, arrow tab appears at saved position and GSAP auto-scrolls to it. User sees exactly where they left off. Works across sessions. Clear button removes bookmark. | `localStorage.setItem('form-bookmark', JSON.stringify({ url, scrollY, fieldId }))`. GSAP scroll on load. Arrow tab shows "Resume here" label. Critical for long workbooks — child comes back next day, picks up where they stopped. |
...
| **Static export — client-owned site handover** | Astro per-page `prerender = true` (or `output: 'static'` for marketing sites) bundles the site to plain HTML/CSS/JS. Zip the `dist/` folder, hand to client, they own it forever. Hosts free on Netlify/GitHub Pages/their own server. No Cloudflare Worker, no monthly fees, no dependency on us. All accessibility ships in the export — themes, render modes, hover modes, gates, zones, screen-reader semantics, keyboard navigation, GSAP animations, theme switcher, Your View panel, AAC pictograms. Phosphor icons download free, drop in `public/icons/`. Source code stays ours; client gets the rendered output. Two service tiers: hosted-with-us (dynamic, themes update) vs bought-outright (frozen handover). | Per-page opt-in via `export const prerender = true`. Audit which routes are static-friendly (most marketing pages are). Login/cart need third-party services (Stripe, Formspree etc.) to keep the export self-contained. Icon.astro needs build-time flag to read from local path instead of API. |
