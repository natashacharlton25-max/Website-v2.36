...
| LottieIcon | PASS | 2026-03-10 | **Pass 2 (2026-03-10):** `class:list` migration — string concatenation replaced with `class:list={['lottie-icon', className]}`. No internal tokens needed (CSS uses `currentColor` inheritance only). No colour group needed (parent owns colour). No render mode CSS needed (pipeline routes to Icon/Text atoms). ACCEPTED: inline px size (same as Icon), `data-semantic-role="decorative"` hardcoded (cross-atom conditionality deferred). **Pass 1 (2026-03-04):** Schema: three optional content dimensions — slug/src (animation), fallbackIcon (pipeline → Icon atom), label (aria-label + pipeline → Text atom). DEFERRED: Consumer migration from src paths to slugs, lottie_mappings shared fallback verification, JS bundle gating in non-full renders. |
...
| List | PASS | 2026-03-10 | **Pass 2 (2026-03-10):** 3 internal `--_list-*` tokens added to `.list` base (marker, marker-accent, text). All raw bridge chains replaced (9 occurrences across default marker, icon, dot, assistive ::before, textonly markers). Schema `iconColor` enum fixed: `[primary, primary-dark, secondary, text, inherit]` → `[primary, accent, text, inherit]` matching CSS classes. `colour` group added (3 pipeline tokens: listMarker, listMarkerAccent, listText). Class array cleaned (falsy-friendly `&&`, no `.filter(Boolean)`). Schema notes updated with render behaviour. **Pass 1 (2026-03-05):** Fixes 1-10 applied. Schema: category → "atom", 4 render keys, props grouped. @layer removed. Dot sizes em-based. a11y.css extracted. DEFERRED: Icon inheritance (cross-atom), slot consumer migration, print (global layer). |
| Text | PASS | 2026-03-10 | **Pass 2 (2026-03-10):** 3 phantom tokens fixed: `--text-color` → `--text-body`, `--text-accent` → `--brand-c-secondary`, `--text-link` → `--link-color`. Schema `color` enum fixed: `[text, text-light, primary, secondary, neutral, inherit]` → `[text, muted, accent, link, inherit]` matching CSS classes. Class array cleaned (falsy-friendly `&&`, no `.filter(Boolean).join(' ')`, `class:list` on element). Schema notes updated with render behaviour and token consumption. No internal tokens needed — Text consumes global text tokens directly. No colour group needed — `color` prop handles pipeline control via class selection. **Pass 1 (2026-03-06):** Schema corrected, font family fallbacks stripped, context overrides deleted, blockquote tokenized, `textTone` prop added. DEFERRED: Card/nav consumer prop verification, --font-body-alt/--font-handwriting brand coverage. |
...
| Tooltip | PASS | 2026-03-10 | **Pass 2 (2026-03-10):** Major CSS consolidation. `--_tooltip-arrow` and `--_tooltip-shadow` internal tokens added. `--_tooltip-accent` added for neon theme. All per-position arrow overrides deleted from glass/neon/brutalist (~50 lines) — themes set internal tokens, base position rules consume them. Assistive render routed through `--_tooltip-bg`/`--_tooltip-text`/`--_tooltip-border`. Glass `blur(20px)` tokenised → `var(--glass-blur-heavy)`, `saturate(160%)` kept as documented exception. `font-weight: 500` → `var(--font-medium)`. `font-family: monospace` → `var(--font-mono)`. Neon text-shadow/box-shadow routed through `--_tooltip-accent`/`--_tooltip-shadow`. Schema `colour` group added (8 pipeline tokens). Class array cleaned (`.filter(Boolean)` removed). Dark mode verified — tooltip inherits zone overrides via bridge tokens. **Pass 1 (2026-03-07):** Built from spec. DEFERRED: utilities.css tooltip block coexistence, script bundle gating in non-full renders. |
...
| Menu/ShareMenu | PASS | 2026-03-10 | **Full build (2026-03-10):** New molecule — modal share panel using native `<dialog>`. Composes Button, Icon, LottieIcon, Link, Heading, Tooltip atoms. Platforms loaded from `BRAND_CONFIG.sharing.platforms` with per-instance override. Lottie animated icons (API slug) with Phosphor static fallback (`:has()` CSS toggle). Two triggers: text Button (desktop) + icon button (micro). Copy-link: full input section + compact icon button. Responsive cascade: 767→380→300→250→200px (4-col→2-col→compact→text-links→icon-only). Tooltip dialog fix: CSS `position: absolute` inside dialogs (replaces broken `position: fixed` in top layer). ResizeObserver on tooltip triggers for text-size/zoom reposition. Dark mode: tooltip contrast + close button primary circle in `theme-luminance-dark.css`. High contrast: tooltip solid border + 3px focus outlines + close button primary circle in `high-contrast.css`. No `!important`, no `@layer`, no `.a11y-*`. Schema: molecule, 4 render keys, content/visual/animation groups. Files: ShareMenu.astro, ShareMenu.css, ShareMenu.responsive.css, ShareMenu.schema.json, index.ts. |
